...
def fix_vaccination_records_table():
    """Drop and recreate the vaccination_records table with correct schema"""
    try:
        # Drop the existing table if it exists
        with engine.connect() as connection:
            connection.execute(text("DROP TABLE IF EXISTS vaccination_records CASCADE"))
            connection.commit()
            print("✅ Dropped existing vaccination_records table")
        
        # Create the table with correct schema
        Base.metadata.create_all(bind=engine)
        print("✅ Vaccination records table recreated successfully!")
        
        # Create sample data
        create_sample_vaccination_records()
        
    except Exception as e:
        print(f"❌ Error fixing vaccination records table: {e}")
